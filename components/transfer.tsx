...
const Transfer = () => {
  ...
  const omnichainSwapContractAddress = "0x8dca049b210c9c27341acbd072357e03d491913a"
  ...
  useEffect(() => {
    if (fees) {
      if (sendType === "crossChainZeta") {
        ...
        console.log("destinationTokenSelected",destinationTokenSelected);
  ...
  m.crossChainSwapHandle = async (action: string) => {
    ...
    console.log("omnichainSwapContractAddress",omnichainSwapContractAddress);
  ...
  const handleSend = async () => {
    ...
    try {
      console.log("sendType as keyof typeof m",sendType as keyof typeof m);
  ...
  // console.log("destinationBalances",destinationBalances);
